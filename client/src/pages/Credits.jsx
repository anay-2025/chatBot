...
const Credits = () => {
  ...
  const fetchPlans = async () => {
    try {
      ...
      if(data.success) {
        ...
        toast.error(data.message || 'Failed to fecth plans')
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (params.get('payment') === 'success') {
      const sessionId = params.get('session_id');
        axios.post('/api/credit/verify', { sessionId }, { headers: { Authorization: token } })
            .then(({ data }) => {
                if (data.success) {
  ...
  return (
    ...
    mx-auto px-4 sm:px-6 lg:px-8 py-12'>
      <h2 className='text-3xl font-semihold text-center mb-10
      xl:mt-30 text-gray-800 dark:text-white'>Credit Plans</h2>

      <div className='flex flex-wrap justify-center gap-8'>
        {plans.map((plan) => (
          <div key={plan._id} className={`border border-gray-200
          dark:border-purple-700 rounded-lg shadow hover:shadow-lg
          tarnsition-shadow p-6 min-w-[300px] flex flex-col ${plan._id === 'pro' ? 
